...
export const useAppStore = create<AppState>()((set) => {
  return {
    ...
    updateDailyReport: (dateStr, report) => {
      set((state) => {
        ...
        import('../lib/syncEngine').then(({ cloudSaveDateReportNow }) => {
          cloudSaveDateReportNow(dateStr, report);
        });
        ...
        Object.keys(nextReports).forEach(dateStr => {
          ...
          if (report.extracted) {
            report.extracted = report.extracted.map(item => {
              if (item.godown === oldName) { changed = true; return { ...item, godown: newName }; }
              return item;
            });
          }
          if (report.final) {
    ...
    removeGodown: (name) => {
      set((state) => {
        ...
        Object.keys(nextReports).forEach(dateStr => {
          ...
          if (report.extracted) {
            report.extracted = report.extracted.map(item => {
              if (item.godown === name) { changed = true; return { ...item, godown: '' }; }
              return item;
            });
          }
          if (report.final) {
    ...
    logActivity: (action, details) => {
      set((state) => {
        // Find current user safely without importing useAuthStore to avoid circular deps
        // We can just rely on configuredUsers and local storage, but easier is extracting from useAuthStore lazily
        let userName = 'Unknown';
        let userEmail = 'unknown@example.com';
        try {
          // Lazy load useAuthStore to avoid circular dependency
          ...
          }).catch(e => logger.warn('store', 'logActivity', 'Failed to lazy load useAuthStore', e));
        } catch (e) {
          logger.warn('store', 'logActivity', 'Failed to parse auth user for log', e);
        }
    ...
    renameSupplier: (oldName, newName) => {
      set((state) => {
        ...
        Object.keys(nextProductMaster).forEach(k => {
          const entry = { ...nextProductMaster[k] };
          if (entry.refillBy === oldName) {
            entry.refillBy = newName;
            nextProductMaster[k] = entry;
            pmChanged = true;
          }
        });
        ...
        Object.keys(nextReports).forEach(dateStr => {
          ...
          if (report.extracted) {
            report.extracted = report.extracted.map(item => {
              if (item.refillBy === oldName) { changed = true; return { ...item, refillBy: newName }; }
              return item;
            });
          }
          if (report.final) {
    ...
    removeSupplier: (name) => {
      set((state) => {
        ...
        Object.keys(nextReports).forEach(dateStr => {
          ...
          if (report.extracted) {
            report.extracted = report.extracted.map(item => {
              if (item.refillBy === name) { changed = true; return { ...item, refillBy: '' }; }
              return item;
            });
          }
          if (report.final) {
