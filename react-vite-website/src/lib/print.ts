...
function buildRefillPdf(list: any[], meta: any) {
  ...
  const mStr = `${meta.date || '-'}  |  ${meta.staff === 'ALL' ? 'All Users' : (meta.staff || 'All')}  |  ${meta.status === 'ALL' ? 'All' : (meta.status || 'All')}  |  ${meta.printed || nowStamp()}`;
  ...
  gKeys.forEach(gKey => {
    body.push([{ 
      content: gKey, 
      colSpan: 7, 
      styles: { fillColor: [225, 235, 245], textColor: [20, 50, 100], fontStyle: 'bold', halign: 'left', fontSize: 7, cellPadding: { top: 2, bottom: 1.5, left: 2, right: 2 } } 
    }]);
    ...
    groups[gKey].forEach((t, i) => {
      ...
      const stock = Math.round(t.balanceQty != null ? t.balanceQty : (Number(t.opening||0) + Number(t.purQty||0) - Number(t.soldQty||0)));
      ...
      body.push([
        String(i + 1),
        { content: prod, _stock: !isNaN(stock) ? `[Stock: ${stock}]` : '' },
        brand,
  ...
  autoTable(doc, {
    ...
    head: [['#', 'Product', 'Brand', 'Refill', 'MRP/Selling', 'User', 'Status']],
    body: body,
    theme: 'grid',
    showHead: 'everyPage',
    rowPageBreak: 'avoid',
    styles: { fontSize: 6, cellPadding: 0.6, valign: 'middle', lineColor: [180, 180, 180], lineWidth: 0.1, textColor: [0, 0, 0], overflow: 'linebreak' },
    headStyles: { fillColor: [230, 230, 230], textColor: [0, 0, 0], fontSize: 6.5, fontStyle: 'bold', halign: 'left', cellPadding: 1 },
    columnStyles: {
      0: { cellWidth: 6,  halign: 'center' },
      1: { fontStyle: 'bold', cellPadding: { top: 0.6, right: 14, bottom: 0.6, left: 0.6 } },
      2: { cellWidth: 16, halign: 'center' },
      3: { cellWidth: 9, halign: 'center', fontStyle: 'bold', textColor: [0, 0, 0], fontSize: 6.5 },
      4: { cellWidth: 18, fontSize: 6.5, fontStyle: 'bold', textColor: [0, 0, 0], halign: 'center' },
    ...
    didDrawCell: function (data: any) {
      if (data.section === 'body' && data.column.index === 1 && data.cell.raw && data.cell.raw._stock) {
        doc.setFont('helvetica', 'italic');
        doc.setFontSize(5.5);
        doc.setTextColor(120, 120, 120);
        const stockHint = data.cell.raw._stock;
        const textWidth = doc.getTextWidth(stockHint);
        const x = data.cell.x + data.cell.width - textWidth - 0.6;
        const y = data.cell.textPos ? data.cell.textPos.y : (data.cell.y + 2.5);
        doc.text(stockHint, x, y);
      }
    },
    didDrawPage: function (data: any) {
      ...
      doc.text(str, pageWidth - M - doc.getTextWidth(str), pageHeight - 1.5);
    }
  });
