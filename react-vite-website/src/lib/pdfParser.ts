...
async function processPDFProxy(pdf: pdfjsLib.PDFDocumentProxy): Promise<{ dateStr: string; items: ExtractedItem[]; srRowsSeen: number }> {
  ...
  // -- Step 2b: Validate this is a stock report, not a random PDF --
  
  // Must contain at least some stock-report column headers
  const stockSignatures = [
    // Column headers that should appear in a Softcon stock report
    { keywords: ['opening', 'opn', 'op.qty', 'op qty', 'open'], label: 'Opening Qty' },
    { keywords: ['balance', 'bal', 'closing', 'cl.qty', 'cl qty', 'close'], label: 'Balance/Closing' },
    { keywords: ['sr.', 'sr', 's.no', 'sl no', 'sl.no'], label: 'Sr. No' },
  ];

  // Must match at least 2 of 3 signature groups
  let signatureHits = 0;
  for (const sig of stockSignatures) {
    if (sig.keywords.some(kw => {
  ...
  // Also check for brand/product indicators
  const hasProductIndicators = /\b(brand|product|item|particulars|description|pur\s*qty|sold\s*qty|sale\s*qty|mrp|sale\s*rate)\b/i.test(fullText);
  
  if (signatureHits < 2 && !hasProductIndicators) {
    throw new Error(
      "This does not appear to be a valid stock report PDF. " +
      "Please upload a Softcon daily stock status report containing product rows with Opening, Purchase, Sold, and Balance columns."
    );
  }

  // Reject known non-stock documents
  const rejectionPatterns = [
    /\b(account\s*statement|bank\s*statement|transaction\s*history)\b/i,
    /\b(credit\s*card|debit\s*card|cheque|ifsc|micr)\b/i,
    /\b(salary\s*slip|pay\s*slip|payslip)\b/i,
    /\b(invoice\s*to|bill\s*to|tax\s*invoice|gst\s*invoice)\b/i,
    /\b(aadhaar|pan\s*card|voter\s*id|passport)\b/i,
  ];

  for (const pattern of rejectionPatterns) {
    if (pattern.test(fullText)) {
      throw new Error(
        "This PDF appears to be a personal/financial document (bank statement, invoice, ID, etc.), not a stock report. " +
        "Please upload only Softcon daily stock status reports."
      );
    }
  }
  ...
  if (!currentReport) {
    ...
    extractedProducts.forEach(newProd => {
      ...
      if (existingIdx === -1) {
        ...
        // Always update the live quantities from the new PDF, while preserving the processing state
