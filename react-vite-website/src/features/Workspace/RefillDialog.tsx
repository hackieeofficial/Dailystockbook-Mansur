...
export const RefillDialog: React.FC<RefillDialogProps> = ({ item, onClose, dateStr }) => {
  const { configuredGodowns, configuredUsers, updateDailyReport, updateProductMasterEntry } = useAppStore();
  ...
  useEffect(() => {
    if (item) {
      ...
      const memory = pm[item.id];
      const memoryGodown = memory?.godown || item.godown || '';
  ...
  const content = (
    <div className="flex flex-col gap-4 py-1">
      ...
      <button autoFocus className="opacity-0 absolute w-0 h-0 p-0 m-0" tabIndex={-1} aria-label="hidden-focus-trap" type="button" />
