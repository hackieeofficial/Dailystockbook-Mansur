...
export const FinalList: React.FC<{ dateStr: string }> = ({ dateStr }) => {
  ...
  const { configuredGodowns, updateDailyReport, updateProductMasterEntry } = useAppStore(useShallow(state => ({
  ...
  const handleStatusChange = useCallback((taskId: string, newStatus: string) => {
    ...
    const updated = (stateReport.final || []).map(t => {
      if (t.taskId === taskId) return { ...t, status: newStatus as FinalTaskStatus, updatedAt: Date.now() };
      return t;
    });
    updateDailyReport(dateStr, { ...stateReport, final: updated, _at: Date.now(), _by: 'react-v1' });
  }, [dateStr, updateDailyReport]);
  ...
  const handleBulkComplete = useCallback(() => {
    confirmAction('Mark All Completed', `Update all ${filteredFinal.length} items to "Completed"?`, () => {
      ...
      const updated = (stateReport.final || []).map(t => {
        if (ids.has(t.taskId)) return { ...t, status: 'Completed' as FinalTaskStatus, updatedAt: Date.now() };
        return t;
      });
      updateDailyReport(dateStr, { ...stateReport, final: updated, _at: Date.now(), _by: 'react-v1' });
    });
  }, [dateStr, filteredFinal, updateDailyReport, confirmAction]);
  ...
  const handleGodownChange = useCallback((taskId: string, originalId: string, newGodown: string) => {
    ...
    const updated = (stateReport.final || []).map(t => {
      if (t.taskId === taskId) return { ...t, godown: newGodown, updatedAt: Date.now() };
      return t;
    });

    // Also update the extracted list processedGodown
    const updatedExtracted = (stateReport.extracted || []).map(p => {
      if (p.id === originalId && p.decisionType === 'REFILL') {
        return { ...p, processedGodown: newGodown, decidedAt: Date.now() };
      }
      return p;
    });

    // Also update product master memory
    const pm = useAppStore.getState().productMaster;
    const prev = pm[originalId] || {};
    ...
    updateDailyReport(dateStr, { ...stateReport, final: updated, extracted: updatedExtracted, _at: Date.now(), _by: 'react-v1' });
  }, [dateStr, updateDailyReport, updateProductMasterEntry, user]);
